...
void check_Fad(environment_list* header, declaration_node* tabela[], int len, Fad* node)
{
		...
		if(!strcmp(node->type, "fdef")){
			...
			if(get_args2(node->fdef->fd->list->list)[0]!=0){header->erro=0; return;}
			...
			str2[0] = str2[0] + ('a'-'A'); // passar a minuscula
			if (get_args2(node->fdec->fd->list->list)[0]==0){header->erro=0; return;}
			if(insert_element(tabela, str, str2, header->current, len)){
				environment_node* n = tabela[hash(str, tabela, len)]->function;
				if(n == NULL){
					header->erro=0;
...
void check_Variable_Declaration(environment_list* header, declaration_node* tabela[], int len, Variable_Declaration* node)
{
	...
	do{
		...
		if(insert_element(tabela,str, str2, header->current, len)) {
			...
			if( (strcmp(tabela[hash(str, tabela, len)]->env->env.nome,"Global"))){header->erro=0; printf("Line %d, col %d: Symbol %s already defined\n", tmp->l, tmp->c, str);}
			else if((t[0] != 'd' && str2[0]=='d') || (t[0] == 'd' && str2[0]!='d')){header->erro=0; printf("Line %d, col %d: Conflicting types (got %s, expected %s)\n", tmp->l, tmp->c, str2, t);}
			//free(str2);
			/*
...
void check_If(environment_list* header, declaration_node* tabela[], int len, If* node)
{
	...
	if(str[0]=='d' || str[0]=='u' || str[0]=='v'){header->erro=0; printf("Line %d, col %d: Conflicting types (got %s, expected int)\n", node->ex->l, node->ex->c, str);}
...
void check_Return(environment_list* header, declaration_node* tabela[], int len, Return* node)
{
	...
	else{header->erro=0; printf("Line %d, col %d: Conflicting types (got %s, expected %s)\n", node->l, node->c, str, header->current->env.ret);}
}
	...
	if(!flag){	
		...
		if(c){
			if(c == node->tipo[0]);
				...
				if(c != 'v'){header->erro=0; printf("Line %d, col %d: Conflicting types (got %s, expected %s)\n", node->l, node->c, node->tipo, s);}
			}
		}
	}
	return node->tipo;
...
char* check_Not(environment_list* header, declaration_node* tabela[], int len, Not* node)
{
	...
	else if(node->tipo[0] == 'v'){header->erro=0; printf("Line %d, col %d: Operator ! cannot be applied to type void\n", node->l, node->c);}
	node->tipo = "int";
	return node->tipo;
}
...
char* check_Minus(environment_list* header, declaration_node* tabela[], int len, Minus* node)
{
	if(node==NULL) return "";
	node->tipo = check_Expression(header, tabela, len, node->ex, 1, NULL);
	if(node->tipo[0] == 'v'){header->erro=0; printf("Line %d, col %d: Operator - cannot be applied to type void\n", node->l, node->c);}
...
char* check_Plus(environment_list* header, declaration_node* tabela[], int len, Plus* node)
{
	...
	node->tipo = check_Expression(header, tabela, len, node->ex, 1, NULL);
	if(node->tipo[0] == 'v'){header->erro=0; printf("Line %d, col %d: Operator + cannot be applied to type void\n", node->l, node->c);}
	else if(node->tipo[0] == 'u'){header->erro=0; printf("Line %d, col %d: Operator + cannot be applied to type undef\n", node->l, node->c);}
	return node->tipo;
}
